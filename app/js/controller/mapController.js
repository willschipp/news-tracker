angular.module('news-app').controller('mapController',['$scope','$mdPanel','$rootScope','$mdDialog','markerService',function($scope,$mdPanel,$rootScope,$mdDialog,markerService) {
  ...
  socket.on('connect',function() {
    ...
    socket.on('news',function(payload) {
      //have a new marker, need to add it
      var keys = Object.keys(payload);
      var key = keys[0];
      key = key.replace(/\-/g,'');
      // console.log(key);
      $scope.markers.push(payload[keys[0]]);
    });
  });
...
angular.module('news-app').controller('panelDialogController',['$scope','$mdPanel','$rootScope','$state','$mdDialog',function($scope,$mdPanel,$rootScope,$state,$mdDialog) {
